var defaultUrl = localStorageGetItem("api-url") || "https://proxy.eas1.cn/https://api-ide.eas1.cn";
...
function handleResult(data) {
    ...
    $statusLine.html(`${status.description}, Time: ${time}, Memory: ${memory}`);
...
function run() {
    ...
    $("#nav-msg").text("");
    setTimeout(()=>{
        localStorageSetItem('savedSource',sourceValue);
        $("#nav-msg").text("代码已保存√");
    },500);
}
...
$(document).ready(function () {
    ...
    $insertTemplateBtn.click(function (e) {
        if (isEditorDirty && confirm("你确定吗?所作的所有改动都会丢失!")) {
            insertTemplate();
        }
    });
    ...
    require(["vs/editor/editor.main", "monaco-vim", "monaco-emacs"], function (ignorable, MVim, MEmacs) {
        ...
        layout.on("initialised", function () {
            ...
            sourceEditor.setValue(decode(localStorageGetItem('savedSource')));
            $("#nav-msg").text("代码已从本地存储恢复√");
        });
...
var extraApiUrl = "https://proxy.eas1.cn/https://api-ide.eas1.cn/";
